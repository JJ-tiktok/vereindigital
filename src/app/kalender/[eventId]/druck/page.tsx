    ...
    <main className="min-h-screen bg-surface-muted px-4 py-6 text-foreground print:bg-white print:px-0 print:py-0">
      <div className="mx-auto max-w-5xl rounded-2xl bg-surface p-8 shadow-sm print:max-w-none print:rounded-none print:p-0 print:shadow-none">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3 print:hidden">
          <Link className="rounded-lg border border-border px-4 py-2 text-sm font-semibold text-foreground" href={`/kalender/${event.id}`}>
            Zurueck
          </Link>
          <PrintButton />
        </div>
        ...
        <header className="border-b border-border pb-6">
          <p className="text-xs font-bold uppercase tracking-[0.24em] text-primary">Trainingsplan</p>
        ...
        <section className="grid gap-4 border-b border-border py-6 md:grid-cols-[1fr_220px]">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-muted">Ziel der Einheit</p>
            <p className="mt-2 text-lg font-semibold">{event.trainingPlan?.objective || "Noch kein Ziel hinterlegt."}</p>
            {event.trainingPlan?.notes ? <p className="mt-3 whitespace-pre-line text-sm leading-6 text-foreground">{event.trainingPlan.notes}</p> : null}
          </div>
          <div className="rounded-xl bg-surface-muted p-4 print:bg-slate-50">
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-muted">Umfang</p>
            <p className="mt-2 text-3xl font-black">{totalDuration || "-"} Min.</p>
            <p className="mt-1 text-sm text-muted">{event.trainingPlan?.exercises.length ?? 0} Uebungen</p>
          </div>
        </section>
        ...
        <section className="space-y-6 py-6">
          {event.trainingPlan?.exercises.length ? (
            event.trainingPlan.exercises.map((planExercise, index) => (
              <article className="break-inside-avoid rounded-xl border border-border p-4" key={planExercise.id}>
                <div className="grid gap-5 md:grid-cols-[280px_1fr]">
                  ...
                  <div>
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div>
                        <p className="text-xs font-bold uppercase tracking-[0.18em] text-primary">Teil {index + 1}</p>
